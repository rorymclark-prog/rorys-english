...
test("a fresh week gives the full allowance", () => {
  const b = load();
  assert.equal(b.WEEKLY_MINUTES, 15);
  assert.equal(b.secondsLeft("ferdi", wednesday), 900);
  assert.equal(b.minutesLeft("ferdi", wednesday), 15);
});
...
test("time spoken comes off this week and comes back next week", () => {
  ...
  assert.equal(b.secondsLeft("ferdi", wednesday), 300);
  b.recordSeconds("ferdi", 300, wednesday);
  assert.equal(b.secondsLeft("ferdi", wednesday), 0);
  assert.equal(b.secondsLeft("ferdi", nextMonday), 900, "Monday starts again");
});
...
test("the counter never runs past the allowance or below zero", () => {
  ...
  assert.equal(b.secondsLeft("ferdi", wednesday), 0);
  assert.equal(JSON.parse(b.localStorage.getItem(KEY)).seconds, 900);
});

test("a part-minute is not offered as a whole minute", () => {
  const b = load();
  b.recordSeconds("ferdi", 851, wednesday);   // 49 seconds left
...
test("rubbish in device storage counts as nothing used, never everything", () => {
  ...
  for (const raw of ["not json", '"a string"', "[1,2,3]", '{"week":"2026-09-21"}', '{"week":"2026-09-21","seconds":-5}', '{"week":"2026-09-21","seconds":"lots"}']) {
    ...
    assert.equal(b.secondsLeft("ferdi", wednesday), 900, raw);
  }
});
...
test("each student has their own allowance", () => {
  ...
  assert.equal(b.secondsLeft("valentin", wednesday), 900);
});
...
test("the speaking screen actually spends the allowance", () => {
  assert.match(speak, /const allowance = teacherTest \|\| !studentId \? 900 : secondsLeft\(studentId\)/, "start() reads what is left");
  assert.match(speak, /if \(allowance <= 0\) \{ setStatus\(OUT_OF_MINUTES\); return; \}/, "and refuses when the week is spent");
  assert.match(speak, /capSeconds\.current = Math\.min\(900, allowance\)/, "a call cannot outrun what is left");
...
test("running out invites a request rather than scolding", () => {
  ...
  assert.match(message, /for this week/, "Rory: it should say for this week");
  assert.match(message, /Ask Rory for more/, "Rory: and they can request more");
