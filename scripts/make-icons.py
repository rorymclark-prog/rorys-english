...
Draws a simple charcoal tile with an indigo plate and stone "RE" monogram (VOLTSTONE palette). Output PNGs are committed
...
def draw_icon(size, maskable=False):
    ...
    # charcoal bleed already satisfies that. Draw an indigo rounded plate + monogram.
    pad = int(size * (0.20 if maskable else 0.14))
    plate = [pad, pad, size - pad, size - pad]
    radius = int(size * 0.18)
    d.rounded_rectangle(plate, radius=radius, fill=INDIGO)
    text = "RE"
    font = load_font(int(size * 0.34))
    tb = d.textbbox((0, 0), text, font=font)
    tw, th = tb[2] - tb[0], tb[3] - tb[1]
    d.text(((size - tw) / 2 - tb[0], (size - th) / 2 - tb[1]), text, font=font, fill=STONE)
    return img
