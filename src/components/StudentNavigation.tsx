...
export default function StudentNavigation(){
  ...
  return <aside className="re-sidebar">
    ...
    <div className="re-workspace-label"><BookIcon width={19}/><div>YOUR LEARNING SPACE<strong>{displayName}</strong></div></div>
    ...
    <div className="re-sidebar-bottom"><Link href={`${base}/settings/`} className="re-settings-link"><GearIcon width={20}/>Settings</Link><div className="re-profile"><span>{displayName[0]}</span><div><strong>{displayName}</strong><small>Your English space</small></div></div></div>
  </aside>;
}
