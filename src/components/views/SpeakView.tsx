...
export default function SpeakView({ lines }: { lines: string[] }) {
  ...
  return <VoiceStudio code={code} lines={lines} />;
}
export function VoiceStudio({ code, lines, teacherTest = false }: { code: string; lines: string[]; teacherTest?: boolean }) {
  ...
  async function start() {
    ...
    try {
      ...
      const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/json" }, signal: controller.current.signal, body: JSON.stringify({ code, token, teacherTest, preview, topic: topics[topic].id, sdp: connection.localDescription?.sdp }) });
