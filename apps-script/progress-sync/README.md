...
- Existing student/parent codes and the teacher Script Property password still work. Without “Keep me signed in”, a code session lasts up to six hours and ends when the app is closed; rotating a code revokes earlier sessions.
- Teacher “Keep me signed in” issues a 30-day device token after password verification. Only its hash is stored in durable Script Properties (not the evictable six-hour cache); every request checks expiry and the current password version. Logout deletes that record; password changes revoke remembered sessions. At most 20 remembered devices are retained. Students and parents signing in with an access code can now tick the same box: their token is prefixed `rs_`, stored the same hashed way under `remembered_student_`, capped at 5 devices per code, and revoked by logout or by replacing that access code. A remembered student token can never resolve to a teacher session. The browser stores a bearer session, never a password; use this only on a private device. Student Firebase persistence and ID-token lifetimes are unchanged.
